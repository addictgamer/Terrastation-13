...
/obj/item/paper
	...
	Topic(href, href_list)
		...
		if (href_list["write"])
			var
				id = href_list["write"]
				t = strip_html_simple(input(usr, "What text do you wish to add to " + (id=="end" ? "the end of the paper" : "field "+id) + "?", "[name]", null),8192) as text
				obj/item/i = usr.equipped() // Check to see if he still got that darn pen, also check if he's using a crayon or pen.
				iscrayon = 0
