...
/datum/construction/proc/construct_message(step, mob/user)
	if(Co_VIS_MSG in step)
		user.visible_message(fixText(step, step[Co_VIS_MSG],user, "verb"), fixText(step, step[Co_VIS_MSG],user,"verb", 1))

/datum/construction/proc/start_construct_message(step, mob/user, atom/movable/used_atom)
	if(Co_START_MSG in step)
		user.visible_message(fixText(step, step[Co_START_MSG],user, "start"), fixText(step, step[Co_START_MSG],user,"start",1))
...
//			delay = given_step[Co_DELAY]
	if(delay > 0)
		...
		if(!do_after(user, delay, needhand = 1, target = src.holder))
			return 0
...
/datum/construction/reversible/construct_message(step, mob/user, diff, override)
	...
	if(message_step)
		user.visible_message(fixText(message_step, message_step[Co_VIS_MSG],user, "verb"), fixText(message_step, message_step[Co_VIS_MSG],user,"verb", 1))
...
/datum/construction/reversible/add_max_amounts()
	...
	//NOT IN PLACE: message segments like verbs can be written as {UN|forwardmessage|backwardmessage}. This formats that in selection
		/*
/datum/construction/reversible/fixText(step, message, mob/user, prefix, self = 0)
	return ..(step, message, user, prefix, self)
		while("{UN|" in text)
