...
/proc/generateHoloMinimaps()
	...
	generateStationMinimap(using_map.zMainStation)
	if(world.maxz >= using_map.zAsteroid)
		generateStationMinimap(using_map.zAsteroid)
	if(world.maxz >= using_map.zDerelict)
		generateStationMinimap(using_map.zDerelict)
	if(world.maxz >= using_map.zSalvageYard)
		generateStationMinimap(using_map.zSalvageYard)
...
/proc/generateMarkers(var/ZLevel)
	...
	if(ZLevel == using_map.zMainStation)
		var/i = 1
		...
		for(var/i = 1 to ((2 * world.view + 1)*WORLD_ICON_SIZE))
			for(var/r = 1 to ((2 * world.view + 1)*WORLD_ICON_SIZE))
				...
				if(tile && tile.loc.holomapAlwaysDraw())
					if((!istype(tile, /turf/space) && istype(tile.loc, /area/mine/dangerous/unexplored)) || istype(tile.loc, /area/asteroid/artifactroom) || istype(tile, /turf/simulated/wall) || istype(tile, /turf/simulated/mineral) || istype(tile, /turf/unsimulated/wall) || (locate(/obj/structure/grille) in tile) || (locate(/obj/structure/window/full) in tile))
						if(using_map.holomap_offset_x.len >= zLevel)
							canvas.DrawBox(HOLOMAP_OBSTACLE, min(i+using_map.holomap_offset_x[zLevel],((2 * world.view + 1)*WORLD_ICON_SIZE)), min(r+using_map.holomap_offset_y[zLevel],((2 * world.view + 1)*WORLD_ICON_SIZE)))
						else
							canvas.DrawBox(HOLOMAP_OBSTACLE, i, r)
					else if (istype(tile, /turf/simulated/floor) || istype(tile, /turf/unsimulated/floor) || istype(tile, /turf/simulated/floor/plating/airless/catwalk))
						if(using_map.holomap_offset_x.len >= zLevel)
							canvas.DrawBox(HOLOMAP_PATH, min(i+using_map.holomap_offset_x[zLevel],((2 * world.view + 1)*WORLD_ICON_SIZE)), min(r+using_map.holomap_offset_y[zLevel],((2 * world.view + 1)*WORLD_ICON_SIZE)))
						else
...
/proc/generateCentcommMinimap(var/filter="all")
	...
	for(var/i = 1 to ((2 * world.view + 1)*WORLD_ICON_SIZE))
		for(var/r = 1 to ((2 * world.view + 1)*WORLD_ICON_SIZE))
			var/turf/tile = locate(i, r, using_map.zCentcomm)
			if(tile && (is_type_in_list(tile.loc, allowed_areas) && !is_type_in_list(tile.loc, restricted_areas)))
				if((!istype(tile, /turf/space) && istype(tile.loc, /area/mine/dangerous/unexplored)) || istype(tile, /turf/simulated/wall) || istype(tile, /turf/simulated/mineral) || istype(tile, /turf/unsimulated/wall) || (locate(/obj/structure/grille) in tile) || (locate(/obj/structure/window/full) in tile) || istype(tile, /turf/simulated/shuttle/wall))
					if(using_map.holomap_offset_x.len >= using_map.zCentcomm)
						canvas.DrawBox(HOLOMAP_OBSTACLE, min(i+using_map.holomap_offset_x[using_map.zCentcomm],((2 * world.view + 1)*WORLD_ICON_SIZE)), min(r+using_map.holomap_offset_y[using_map.zCentcomm],((2 * world.view + 1)*WORLD_ICON_SIZE)))
					else
						canvas.DrawBox(HOLOMAP_OBSTACLE, i, r)
				else if (istype(tile, /turf/simulated/floor) || istype(tile, /turf/unsimulated/floor) || istype(tile, /turf/simulated/floor/plating/airless/catwalk) || istype(tile, /turf/simulated/shuttle/floor))
					if(using_map.holomap_offset_x.len >= using_map.zCentcomm)
						canvas.DrawBox(HOLOMAP_PATH, min(i+using_map.holomap_offset_x[using_map.zCentcomm],((2 * world.view + 1)*WORLD_ICON_SIZE)), min(r+using_map.holomap_offset_y[using_map.zCentcomm],((2 * world.view + 1)*WORLD_ICON_SIZE)))
					else
						canvas.DrawBox(HOLOMAP_PATH, i, r)
		...
		for(var/r = 1 to ((2 * world.view + 1)*WORLD_ICON_SIZE))
			...
			if(tile && tile.loc)
				...
				if(areaToPaint.holomap_color)
					if(using_map.holomap_offset_x.len >= StationZLevel)
						canvas.DrawBox(areaToPaint.holomap_color, min(i+using_map.holomap_offset_x[StationZLevel],((2 * world.view + 1)*WORLD_ICON_SIZE)), min(r+using_map.holomap_offset_y[StationZLevel],((2 * world.view + 1)*WORLD_ICON_SIZE)))
					else
...
*/
	...
	if(StationZLevel == using_map.zMainStation)
		...
		for(var/marker in holomap_markers)
			...
			if(holomarker.z == StationZLevel && holomarker.filter & HOLOMAP_FILTER_STATIONMAP_STRATEGIC)
				if(using_map.holomap_offset_x.len >= StationZLevel)
					strategic_map.Blend(icon(holomarker.icon,holomarker.id), ICON_OVERLAY, holomarker.x-8+using_map.holomap_offset_x[StationZLevel]	, holomarker.y-8+using_map.holomap_offset_y[StationZLevel])
				else
	...
	for(var/marker in holomap_markers)
		...
		if(holomarker.z == StationZLevel && holomarker.filter & HOLOMAP_FILTER_STATIONMAP)
			if(using_map.holomap_offset_x.len >= StationZLevel)
				big_map.Blend(icon(holomarker.icon,holomarker.id), ICON_OVERLAY, holomarker.x-8+using_map.holomap_offset_x[StationZLevel]	, holomarker.y-8+using_map.holomap_offset_y[StationZLevel])
			else
